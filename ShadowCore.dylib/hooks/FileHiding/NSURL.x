...
- (NSURLSessionDataTask *)dataTaskWithURL:(NSURL *)url completionHandler:(void (^)(NSData *data, NSURLResponse *response, NSError *error))completionHandler {
    if(isCallerExternal() && [_shadow isURLRestricted:url]) {
        void (^swallowCompletion)(NSData *, NSURLResponse *, NSError *) = ^(NSData *data, NSURLResponse *response, NSError *error) {
            // swallow — the blocked completion below is the only one delivered
        };
        NSURLSessionDataTask* task = %orig(url, swallowCompletion);
...
- (NSURLSessionDownloadTask *)downloadTaskWithURL:(NSURL *)url completionHandler:(void (^)(NSURL *location, NSURLResponse *response, NSError *error))completionHandler {
    if(isCallerExternal() && [_shadow isURLRestricted:url]) {
        void (^swallowCompletion)(NSURL *, NSURLResponse *, NSError *) = ^(NSURL *location, NSURLResponse *response, NSError *error) {
            // swallow
        };
        NSURLSessionDownloadTask* task = %orig(url, swallowCompletion);
...
- (NSURLSessionUploadTask *)uploadTaskWithRequest:(NSURLRequest *)request fromFile:(NSURL *)fileURL completionHandler:(void (^)(NSData *data, NSURLResponse *response, NSError *error))completionHandler {
    if(isCallerExternal() && (_shdw_requestRestricted(request) || [_shadow isURLRestricted:fileURL])) {
        ...
        NSURLSessionUploadTask* task = %orig(request, fileURL, swallowCompletion);
...
- (NSURLSessionDataTask *)dataTaskWithRequest:(NSURLRequest *)request completionHandler:(void (^)(NSData *data, NSURLResponse *response, NSError *error))completionHandler {
    if(isCallerExternal() && _shdw_requestRestricted(request)) {
        ...
        NSURLSessionDataTask* task = %orig(request, swallowCompletion);
...
- (NSURLSessionDownloadTask *)downloadTaskWithRequest:(NSURLRequest *)request completionHandler:(void (^)(NSURL *location, NSURLResponse *response, NSError *error))completionHandler {
    if(isCallerExternal() && _shdw_requestRestricted(request)) {
        ...
        NSURLSessionDownloadTask* task = %orig(request, swallowCompletion);
